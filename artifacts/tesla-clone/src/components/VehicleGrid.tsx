...
const vehicles: Vehicle[] = [
  {
    ...
    img: `${BASE}hero-model-3.jpg`, imgPos: 'center 52%',
    colors: [
  ...
  },
  {
    ...
    img: `${BASE}hero-model-3.jpg`, imgPos: 'center 48%',
    colors: [
      { name: 'Pearl White', hex: '#f0f0ec', filter: 'none' },
      { name: 'Stealth Grey', hex: '#4a4a4a', filter: 'brightness(0.55) saturate(0.12)' },
      { name: 'Midnight Silver', hex: '#737373', filter: 'brightness(0.68) saturate(0.18)' },
  ...
  },
  {
    ...
    img: `${BASE}hero-highway.jpg`, imgPos: 'center 40%',
    colors: [
...
function useDragScroll() {
  ...
  useEffect(() => {
    ...
    const down = (e: MouseEvent) => {
      st.current = { down: true, startX: e.pageX - el.offsetLeft, scrollLeft: el.scrollLeft, moved: false };
      el.style.cursor = 'grabbing'; el.style.userSelect = 'none';
    };
    const up = () => { st.current.down = false; el.style.cursor = 'grab'; el.style.userSelect = ''; };
    const move = (e: MouseEvent) => {
      if (!st.current.down) return; e.preventDefault();
      const dx = e.pageX - el.offsetLeft - st.current.startX;
      if (Math.abs(dx) > 4) st.current.moved = true;
      el.scrollLeft = st.current.scrollLeft - dx * 1.2;
    };
...
function Card({ v, isLast, idx }: { v: Vehicle; isLast: boolean; idx: number }) {
  ...
  const { containerRef, bgRef } = useParallax(0.24);
  const { ref: carousel, wasDragging } = useDragScroll();

  /* Intersection observer wired into the containerRef */
  ...
  return (
    <div ref={setRef} style={{
      position: 'relative',
      height: isLast ? '54vh' : '84vh',
      minHeight: isLast ? '400px' : '560px',
      overflow: 'hidden',
    ...
    >
      ...
      <div ref={bgRef} style={{
        ...
        {/* ─ Inner div: Ken Burns scale only — never conflicts with parallax ─ */}
        <div style={{
      ...
      <div style={{
        ...
        background: 'linear-gradient(to bottom, transparent 22%, rgba(0,0,0,.48) 58%, rgba(0,0,0,.84) 100%)',
      }} />
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 3, padding: '0 0 18px' }}>
        ...
        <div ref={carousel} style={{
          ...
          padding: '0 18px 10px', scrollbarWidth: 'none', msOverflowStyle: 'none',
          ...
          {v.trims.map((trim, ti) => {
            ...
            return (
              <button key={trim.name} onClick={() => { if (!wasDragging()) setTrimIdx(ti); }} style={{
                flexShrink: 0, scrollSnapAlign: 'start', minWidth: '172px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '0 18px 10px' }}>
          <span style={{ fontSize: '11px', color: 'rgba(255,255,255,.42)', minWidth: '80px' }}>
