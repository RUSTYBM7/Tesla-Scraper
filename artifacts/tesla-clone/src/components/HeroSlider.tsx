...
const slides: Slide[] = [
  {
    ...
    btnPrimary: 'Demo FSD (Supervised)', btnSecondary: 'Learn More',
  ...
  },
  {
    ...
    btnPrimary: 'Order Now', btnSecondary: 'Learn More',
    btnPrimaryStyle: 'dark',
    img: `${BASE}fsd-night.jpg`, imgPos: 'center 40%',
    textLight: true,
  },
];
...
export default function HeroSlider() {
  ...
  /* Parallax for hero — scrollY based since hero is always at page top */
  useEffect(() => {
    let raf: number | null = null;
    const update = () => {
      raf = null;
      if (bgGroupRef.current) {
        bgGroupRef.current.style.transform = `translateY(${(window.scrollY * 0.38).toFixed(2)}px)`;
      }
    };
    ...
    <section style={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden', background: '#0a0a0a' }}>
      ...
      <div style={{
        ...
        background: slide.textLight
          ? 'linear-gradient(180deg,rgba(0,0,0,.3) 0%,rgba(0,0,0,.04) 42%,rgba(0,0,0,.2) 100%)'
      ...
      {/* Headline */}
      <div style={{
        position: 'absolute', top: '13%', left: 0, right: 0, textAlign: 'center', zIndex: 2,
        padding: '0 20px',
        opacity: fade ? 1 : 0, transform: fade ? 'translateY(0)' : 'translateY(12px)',
        transition: 'opacity .5s ease .08s, transform .5s ease .08s',
      }}>
        <h1 style={{
          fontSize: 'clamp(22px, 3.5vw, 52px)', fontWeight: 600, color: tc,
          letterSpacing: '-0.3px', lineHeight: 1.15, marginBottom: '10px',
          textShadow: slide.textLight ? '0 1px 8px rgba(0,0,0,.4)' : 'none',
        }}>{slide.title}</h1>
        <p style={{
          fontSize: '15px', color: sc, marginBottom: '26px',
          textShadow: slide.textLight ? '0 1px 4px rgba(0,0,0,.3)' : 'none',
        }}>{slide.subtitle}</p>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <a href="#" style={{
            padding: '12px 40px', borderRadius: '4px', fontSize: '14px', fontWeight: 500,
            background: slide.btnPrimaryStyle === 'blue' ? '#3e6ae1' : 'rgba(23,26,32,.84)',
            color: '#fff', minWidth: '200px', textAlign: 'center',
          ...
          <a href="#" style={{
            padding: '12px 40px', borderRadius: '4px', fontSize: '14px', fontWeight: 500,
            background: 'rgba(255,255,255,.72)', color: '#171a20', minWidth: '200px', textAlign: 'center',
            backdropFilter: 'blur(6px)', transition: 'background .2s, transform .15s',
          }}
